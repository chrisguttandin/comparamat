...
//    });
//    }
    function dependencyFreeExtract(node) {
      var childNode,
      ...
      for (i = 0; i < length; i += 1) {
          ...
          if (childNode.nodeType === 3) {
              textnodes.push(new textnode(childNode));
          } else {
              dependencyFreeExtract(childNode);
          }
      }        
    }
    //_extract($item);
    dependencyFreeExtract(node);
...
function clean_list(list)
{
    ...
    token = token.replace(/[^a-zA-Z0-9\u0410-\u044F]/g, "");
...
function marcusb_fragcolors_modified(td1, td2, len)
{
...
//      textblocks[1].clear();
      ...
      return {
        original: td1.innerHTML,
        plagiarism: td2.innerHTML
      };
